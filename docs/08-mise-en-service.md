...
| A11 | Que fait le cabinet aujourd'hui quand un appel de fonds déjà envoyé se révèle faux ? Refait-il l'appel, envoie-t-il un correctif, ou autre chose ? | À décider | Cabinet | Pour le Développement, deux options techniques selon la réponse : **(1)** démarrer avec ce qui existe — annuler l'appel émis par un simple changement de statut et en émettre un autre, sans document qui dise au copropriétaire ce qui a été annulé ; **(2)** construire l'avoir avant la mise en service — document propre, référençant l'appel d'origine, figé et envoyable, total ou partiel ligne par ligne. Le mécanisme cible est tranché par la **décision 65** de `docs/06-decisions.md` (chantier « Avoir sur appel ») ; la réponse du cabinet décide seulement s'il doit exister dès le premier appel. |
...
| B5 | Adapter le chargeur du registre privé (`donnees-privees/registre-mamelles-reel.sql`) à la table `proprietaires_notes` | À faire | Concepteur | **[ajout]** Il écrit encore `proprietaires.note`, colonne supprimée par `20260920010000` : il échouerait au rechargement. Fichier personnel, non versionné : pas modifié par le développement. La note va dans `proprietaires_notes`, comme dans le seed. **Après B6bis**, le chargeur doit aussi déclarer son intention par le réglage local à la transaction que le déclencheur de fermeture reconnaît (décision 62), sinon il sera refusé. |
...
| B6bis | Fermer la modification silencieuse du registre : tantièmes, rattachements de lots et regroupements modifiables seulement par des fonctions dédiées, avec motif et trace dans `journal` ; modification directe de ces colonnes refusée en base, gestionnaire compris ; coordonnées d'un propriétaire corrigibles librement, mais tracées | À faire | Développement, puis Concepteur | **Prioritaire, avant B7** : tant qu'il n'est pas fait, un associé déclaré en B7 pourrait modifier le registre réel sans motif ni trace. Les appels déjà émis ne sont pas affectés (instantané figé). Chantier « Fermer la modification silencieuse du registre » et décisions 60 à 62 de `docs/06-decisions.md` : version du registre pour les brouillons, regroupement compris, fermeture par déclencheur. Sur la base réelle, par une migration appliquée comme en B6. |
